...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        console.log(err);
        res.status(500).json({ msg: "there error try again later" })
    }
})
...
router.get("/search", async (req, res) => {
    ...
    try {
        ...
        res.status.json({ msg: "There iserror try again try again later" }, err)
    }
})
...
router.put("/:editId", auth, async (req, res) => {
    ...
    try {
        ...
        let data = await CakeModel.updateOne({ _id: editId, user_id: req.tokenData._id})
        res.json(data)
    }
...
router.delete("/:delId", auth, async (req, res) => {
    ...
    try {
        ...
        let data = await CakeModel.deleteOne({ _id: delId, user_id: req.tokenData._id})
        res.json(data)
    }
